...
// Update Month Totals (Total Income, UPI, Card, Cash)
function updateMonthTotals(entries) {
    const totals = entries.reduce((acc, e) => {
        acc.income += parseFloat(e.total_income) || 0;
        acc.upi += parseFloat(e.upi_amount) || 0;
        acc.card += parseFloat(e.card_amount) || 0;
        acc.cash += parseFloat(e.ap_cash || 0) || 0;
        return acc;
    }, { income: 0, upi: 0, card: 0, cash: 0 });

    const incomeEl = document.getElementById('totalIncomeSum');
    const upiEl = document.getElementById('totalUpiSum');
    const cardEl = document.getElementById('totalCardSum');
    const cashEl = document.getElementById('totalCashSum');

    if (incomeEl) incomeEl.textContent = `₹${formatIndianNumber(totals.income)}`;
    if (upiEl) upiEl.textContent = `₹${formatIndianNumber(totals.upi)}`;
    if (cardEl) cardEl.textContent = `₹${formatIndianNumber(totals.card)}`;
    if (cashEl) cashEl.textContent = `₹${formatIndianNumber(totals.cash)}`;
}
...
async function sendWhatsApp(entry) {
    ...
    // Calculate MTD (Month-to-Date total sales)
    const mtdStartDate = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-01`;
    const { data: mtdData } = await supabaseClient
        .from('daily_entries')
        .select('total_income')
        .gte('date', mtdStartDate)
        .lte('date', entry.date);
    const mtdTotal = mtdData ? mtdData.reduce((sum, e) => sum + parseFloat(e.total_income), 0) : 0;
...
MTD : ₹${formatIndianNumber(mtdTotal)}
...
Description: ${expense.description || 'N/A'}`;
    ...
    // If this is today's entry, mark daily report as sent
    const todayISO = new Date().toISOString().split('T')[0];
    if (entry.date === todayISO) {
        localStorage.setItem('pnp_wa_' + todayISO, 'sent');
    }
